...
const ServiceSalientFeature = () => {
  ...
  const [paginatedSerSF, setpaginatedSerSF] = useState([]);
  ...
  const sorting = (col) =>{
    if(order === "ASC"){
      const sorted = [...paginatedSerSF].sort((a,b)=>
        a[col].toLowerCase() > b[col].toLowerCase() ? 1:-1
      )
      setpaginatedSerSF(sorted);
      setOrder("DSC")
    }
    if(order === "DSC"){
      const sorted = [...paginatedSerSF].sort((a,b)=>
  ...
  const EditService = async (e) => {
    ...
    navigate("/editservicesalientfeature", { state: { id: ids } });
  };
  ...
  const Delt = async (e) => {
    ...
    console.log(data);
    await axios
      .delete(`${baseURL}/servicesalientfeature/deletefeature`, {
        data,
        headers: {
          "Content-Type": "application/json",
      ...
      .then((response) => {
        if (response.data.status === 201 || response.data.status === 401) {
          ...
          axios.get(`${baseURL}/servicesalientfeature/getallfeature`, options).then((response) => {
            // console.log(response.data.data);
            setService(response.data.data);
            setpaginatedSerSF(_(response.data.data).slice(0).take(pageSize).value());
          });
          refresh({});
  ...
  const Paginations = (pageNo) => {
    ...
    const paginatedSSF = _(service).slice(startIndex).take(pageSize).value();
    setpaginatedSerSF(paginatedSSF);
  }
            ...
            <div class="header-body">
              <div class="row align-items-center py-4 pb-7">
                <div class="col-lg-6 col-7">
                  <h6 class="h2 text-white d-inline-block mb-0">Service Salient Features</h6>
            ...
            <div class="col">
              <div class="card">
                ...
                <div class="card-header algn">
                  <h3 class="mb-0">Service Salient Features</h3>
...
export default ServiceSalientFeature;
